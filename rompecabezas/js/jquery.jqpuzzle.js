...
$.fn.jqPuzzle = function(settings, texts) {
	...
	var defaults = {
		...
		hole: 16,					// posicion inicial del agujero de la ficha [1 ... rows*columns]
		shuffle: false,				//  inicialmente muestra kas piezas barajadas[true|false]
		numbers: true,				// inicialmente muestra el numero de piezas [true|false]
		language: 'en',				// lenguaje para los elementos [language code]

		// display additional gui controls
		control: {
			shufflePieces: true,	// mostrar el boton'aleatorio' [true|false]
			confirmShuffle: true,	// preguntar antes de barajar[true|false]
			toggleOriginal: true,	// mostrar el boton 'Original' [true|false]
			toggleNumbers: true,	// mostrar el boton 'Numeros' [true|false]
		...
		success: {
			fadeOriginal: true,		// imagen original del fundido cruzado [true|false]
			callback: undefined,	// devolucion de la funcion por el usuario [function]
									// a la funcion se le pasa un objeto como argumento
									// aqui se incluyen los campos'movimientos' y 'segundos'
			callbackTimeout: 300	// el tiempo en ms despues del cual se llama la devolucion de llamada
		},

		// animation speeds and settings
		animation: {
			shuffleRounds: 3,		// cantidad de rondas barajadas [1 ... ]
			shuffleSpeed: 800,		// tiempo en ms para realizar una ronda aleatoria
			slidingSpeed: 200,		// tiempo en ms para un solo movimiento
			fadeOriginalSpeed: 600	// tiempo en ms para desvanecer la imagen original 
		},

		// informacion del estilo adicional no especificada a traves de css
		style: {
			gridSize: 2,			// espacio entre dos piezas en px
			overlap: true,			// si es cierto, los bordes de las piezas adyacentes se superpondra
									// se aplica solo si gridSize se establece en 0
			backgroundOpacity: 0.1	// opacidad de la imagen original detras de las piezas
	...
	// si las filas o cols pero ningun agujero fue definido por el usuario,
	// establece explicitamente la posicion del agujero en la ultima pieza (bottom right)
	...
	// extender el objeto de configuracion definido por el usuario con la configuracion predeterminada
	settings = $.extend(true, {}, defaults, settings);

	// extender el objeto de textos definidos por el usuario con textos de idioma actuales
	texts = $.extend((i18n[settings.language] || i18n[defaults.language]), texts);

	// crear algunas variables de acceso directo útiles
	...
	// verifique la configuración de valores válidos

	//mantener filas y columnas dentro de los límites
	if(rows < 3 || rows > 9) rows = defaults.rows;
	if(cols < 3 || cols > 9) cols = defaults.rows;

	// mantener la posición del agujero dentro de los límites
	if((hole > (rows*cols)) || (hole < 1)) hole = rows*cols;
	hole--; // zero-based index

	// velocidad de animación = 0 no funciona
	...
	// mantener rondas de barajar dentro de los límites
	...
	var checkSolution = function($pieces) {
		// iterar sobre las piezas y verificar cada pieza
		for(var i = 0; i < $pieces.length; i++) {
			// como el agujero no se guarda en la matriz de piezas,
			// ajustar el índice si está más allá de la posición del agujero
			var pieceIndex = (i < hole) ? i : i + 1;

			// comprobar si la posición actual coincide con la posición objetivo (índice)
			if($pieces.eq(i).attr('current') != pieceIndex) return false;
		}
		return true;
	};

	// comprueba si el rompecabezas se puede resolver(pure math ...)
	...
	// obtener el índice lineal de un par fila / col(zero-based)
	var getLinearPosition = function(row, col) {
		return parseInt(row)*cols + parseInt(col);
	};

	// obtener el par fila / col de un índice lineal (zero-based)
	var getMatrixPosition = function(index) {
		return {row: (Math.floor(index/cols)), col: (index%cols)};
	};

	//obtener el ancho de píxel de un borde (internet explorer returns keywords)
	// se usarán los valores del lado izquierdo
	var getBorderWidth = function($element) {
		// el valor css reportado
		var property = $element.css('border-left-width');
		// Se debe establecer un estilo de borde para obtener un ancho de borde válido
	...
	// pasar un intervalo de tiempo de espera en ms, después del cual se llama a la devolución de llamada
	// la devolución de llamada pasa un argumento, el tiempo transcurrido es ms
	...
	// aplicar jqPuzzle a cada elemento de imagen dentro de la selección -------------------

	return this.filter('img').each(function(){
		var $srcImg = $(this);			// imagen de origen como objeto jQuery
		var lock = false;				// se está ejecutando la bandera de animaciones
 

		var moves = 0;					// contador para movimientos individuales
		var seconds = 0;				// contador por segundos después del primer movimiento
		var solved;						// Bandera del rompecabezas está resuelto por el usuario
		var shuffled = settings.shuffle;// flag if the puzzle was shuffled
		var timer;						// un componente de temporizador

		// guardar la posición actual del agujero para una mayor manipulación
		var currHole = hole;


		// crear elementos ficticios para obtener propiedades css calculadas
		...
		// Reemplazar imagen original con ficticio
 

		$srcImg.replaceWith($dummyGui);

		// Asignar ID de imagen antigua a ficticio
 

		$dummyGui.attr('id', $srcImg.attr('id') || '');

		// obtener propiedades css calculadas de elementos ficticios
		var computedStyles = {
			gui: {
		...
		// Reemplazar elementos ficticios con imagen original
		$dummyGui.removeAttr('id');
		$dummyGui.replaceWith($srcImg);


		// espere a que se cargue la imagen para poder obtener su ancho / alto real
		$srcImg.one('load', function() {

			// se superponen los bordes de las piezas si no hay margen entre las piezas
			// de esta manera, los bordes de las piezas no se duplicarán
			var overlap = (style.gridSize === 0 && style.overlap);

			// espacio total de bordes de piezas y líneas de cuadrícula, que cubrirán partes de la imagen
			...
			// recalcar si se superponen
			if(overlap) {
				coveredWidth  -= (cols-1)*computedStyles.piece.border;
				coveredHeight -= (rows-1)*computedStyles.piece.border;
			}

			// asegúrese de obtener el tamaño de imagen original, no valores escalados
			// en mozilla, width () y height () no funcionan con elementos ocultos
			$srcImg.css({width: 'auto', height: 'auto', visibility: 'visible'});

			// ancho y alto de las piezas, según el tamaño de la imagen original
			var width  = Math.floor(($srcImg.width()-coveredWidth) / cols);
			var height = Math.floor(($srcImg.height()-coveredHeight) / rows);

			// rechazar imágenes demasiado pequeñas
			if(width < 30 || height < 30) return false;

			// Recupere el ancho y la altura de la imagen completa para evitar errores de redondeo
			...
			// ancho / alto total de una pieza (incluido el borde de la pieza y 1 tamaño de cuadrícula)
			...
			// manejar el modelo de caja de modo de peculiaridades de Internet Explorer
			var boxModelHack = {
			...
			// desplazamiento de píxeles de un elemento, basado en la posición de la matriz
			...
			var shuffle = function(rounds, speed) {
				...
				// cuando se define la velocidad, la función fue activada por un evento del usuario (button click)
				if(speed) {
			...
			// opera tiene un efecto extraño al llamar a hide () y val () antes
			// los elementos están unidos a la dom
			// ocultar números
